...
async function handleAddToWishlist(interaction) {
    const appid = interaction.customId.replace('steam_add_to_wishlist_', '');

    try {
        const game = await getGameDetails(appid);
        if (!game) {
            return interaction.reply({ content: 'Could not fetch game details to add to wishlist.', ephemeral: true });
        }
        ...
        const gameData = {
            ...
            price: game.price_overview?.final_formatted || 'Free/N/A'
        };

        const added = await addToWishlist(interaction.user.id, 'games', gameData);

        if (added) {
            await interaction.reply({
                ephemeral: true,
                content: `Successfully added **${game.name}** to your games wishlist! ❤️`
            });
        } else {
            await interaction.reply({
                ephemeral: true,
                content: `**${game.name}** is already in your wishlist.`
            });
        }
    } catch (error) {
        console.error('Error adding game to wishlist:', error);
        await interaction.reply({ content: 'An error occurred while adding the game to your wishlist.', ephemeral: true });
    }
}
...
async function handleGameSearch(interaction) {
    ...
    try {
        ...
        // Add game details
        const fields = [];
        const ga = details.name
        const gaurl = ga.split(' ').join('+');
        const aurl = ga.split(' ').join('%20');
        ...
        embed.addFields({
            name: 'Game3rb Search',
            value: `🔗 [View in Game3rb](https://game3rb.com/?s=${gaurl})`,
            inline: true
        });
        embed.addFields({
            name: 'RexaGames Search',
            value: `🔗 [View in RexaGames](https://rexagames.com/search/?&q=${aurl})`,
            inline: true
        });
        ...
        const row = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    ...
                    .setCustomId(`steam_add_to_wishlist_${game.appid}`)
                    .setEmoji('❤️')
            );
...
// Button handler for showing more search results and wishlist
async function handleButton(interaction) {
    const customId = interaction.customId;

    if (customId.startsWith('steam_add_to_wishlist_')) {
        await handleAddToWishlist(interaction);
        return;
    }

    if (customId.startsWith('steam_more_results_')) {
        await interaction.deferUpdate();

        const query = customId.replace('steam_more_results_', '');
